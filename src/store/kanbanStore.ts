...
export const useKanbanStore = create<KanbanState>((set) => ({
            ...
            const cards: DetailedCardProps[] = mockCards.map(
                (card: DetailedCardProps) => ({
                    ...
                    processDetails: {
                        ...
                        materialStockSize:
                            card.processDetails?.materialStockSize || '',
                        surfaceTreatment:
                            card.processDetails?.surfaceTreatment || '',
    ...
    addLastViewed: (card) =>
        set((state) => {
            const withoutDuplicate = state.lastViewed.filter(
                (c) => c.id !== card.id,
            );
