...
loadCards: async () => {
  try {
    ...
    const initialColumns: Column[] = [
      ...
      { id: 'in-progress-column', title: 'In Progress', cards: cards.slice(5, 10) },
      { id: 'in-review-column', title: 'In Review', cards: cards.slice(10, 15) },
    ...
    moveCard: (sourceColumnId, targetColumnId, sourceIndex, destinationIndex) =>
        set((state) => {
          ...
          if (!sourceColumn || !targetColumn) {
            console.error('Source or Target column not found:', { sourceColumnId, targetColumnId });
            return state;
          }
          ...
          if (!movedCard) {
            console.error('Failed to retrieve card from source column:', { sourceIndex, sourceColumn });
            return state;
          }
          ...
          console.log('Before move:', JSON.stringify(state.columns, null, 2));
          console.log('Source column before move:', sourceColumn);
          console.log('Target column before move:', targetColumn);
        ...
        editCard: (id: string, updatedFields: Partial<DetailedCardProps>) => {
          try {
            set((state) => {
              ...
              localStorage.setItem("kanbanColumns", JSON.stringify(updatedColumns));
              console.log("Updated localStorage:", updatedColumns);
